...
export default function TaskTable() {
  ...
  // Fungsi fetch dipindahkan ke sini
  async function fetchStudentTasks() {
    setLoading(true);
    try {
      // Query ke student_tasks dengan join ke tasks, courses, dan students
              ...
              {tasks.length > 0 ? (
                tasks.map((item) => (
                  <tr key={item.id} className="border-b-4 border-black hover:bg-green-100 transition-colors bg-white text-black">
                    <td className="p-4 border-r-4 border-black font-black text-lg uppercase">
                      {item.students?.nama}
                    </td>
                    ...
                    <td className="p-4 border-r-4 border-black italic text-gray-600">
                      {item.tasks?.courses?.nama_matkul} (Sem {item.tasks?.courses?.semester})
                    </td>
