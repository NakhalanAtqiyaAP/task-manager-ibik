...
export default function Navbar({ onMenuAction, currentUser, onToggleProfile }) {
  ...
  // Profile Dihapus dari array, karena akan jadi Avatar terpisah
  ...
  return (
    <nav className="border-b-4 border-black bg-white sticky top-0 z-50 p-4 md:p-6">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => onMenuAction('Dashboard', 'view')}>
          ...
          <span className="text-2xl md:text-3xl font-black tracking-tighter uppercase hidden sm:block">TI-25-KA</span>
        </div>
        ...
        <div className="flex items-center gap-4 md:gap-6 relative">
              ...
              <div key={item} className="relative group">
                ...
                {activeSub === item && (
                  <div className="absolute top-full mt-2 left-0 w-48 bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] z-[60]">
                    <button onClick={() => handleAction(item, 'view')} className="w-full text-left p-3 font-black uppercase border-b-4 border-black hover:bg-green-400 transition-colors">
                      🔍 Tampilkan Data
                    </button>
                    <button onClick={() => handleAction(item, 'create')} className="w-full text-left p-3 font-black uppercase hover:bg-green-400 transition-colors">
                      ➕ Bikin Data
                    </button>
                  </div>
                )}
          ...
          <button 
            ...
            className="w-12 h-12 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all overflow-hidden bg-purple-200"
            title="Buka Profil"
          >
            <img 
